...
function createHTMLTable($data){
    ...
    foreach($data as $key=>$value){
	    if($i % 2 ==0) {
           $html.="<tr class='even' width='100%'>";
		} else {
		$html.="<tr class='odd' width='100%'>";
		}
...
###################################################################################################
	# JSON RESPONSE																					  #
	###################################################################################################
	 function returnMsg($responseCode, $responseDescription = "", $responseMessage ="" , $responseData = "", $emptyData = "{}")
    {
        $msg = "";
        try {
        	if(count($responseData) > 0)
            	$arrMsg = array('response code' => $responseCode, 'response description' => $responseDescription, 'response message' => $responseMessage, 'response data' => $responseData);
            else
            	$arrMsg = array('response code' => $responseCode, 'response description' => $responseDescription, 'response message' => $responseMessage, 'response data' => json_decode($emptyData));
            $msg = json_encode($arrMsg, true);
        } catch (Exception $e)
        {
            $arrMsg = array('response code' => SN_RETURN_CODE_EXCEPTION, 'response description' => "Encoded failed", json_decode($emptyData));
            $msg = json_encode($arrMsg, true);
