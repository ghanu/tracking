...
class cSales_orders_trackingController extends cController {
 public $action="viewall";
    public $id="";
    public $view_controls=array();
    function __construct() {
        parent::__construct();
        $this->table = "sales_order_tracking";
        $this->action="viewall";
    }
    
 function curd() {
        if($this->action=="viewall"){
            $this->column=array("seq_no,operation_text,work_center,setup_time,run_time,working_hours,
req_days,down_time,start_date,end_date,status,remarks");
			$this->addWhereCondition('id=' . $this->id);
        ...
        $result=parent::curd($this->action,'');
